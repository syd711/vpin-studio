...
public class VPinStudioServerTray {
  ...
  public VPinStudioServerTray() {
    try {
      ...
      // Use absolute path for the resource
      BufferedImage iconImage = ResourceLoader.getResource("/de/mephisto/vpin/server/resources/logo-small.png");
      if (iconImage == null) {
        LOG.error("Failed to load tray icon image from classpath.");
        return;
      }
      ...
      LOG.error("Failed to initialize SystemTray: {}", e.getMessage(), e);
    }
  }
