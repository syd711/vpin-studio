...
public class B2STableSettingsSerializer {
  ...
  public void serializeXml(@NonNull DirectB2ServerSettings settings, @NonNull File xmlFile, boolean add) throws VPinStudioException {
    serializeXml(settings, xmlFile, null, serverEntries, (s, name) -> getServerValue(s, name), add);
  }

  public void serializeXml(@NonNull DirectB2STableSettings settings, @NonNull File xmlFile, boolean add) throws VPinStudioException {
    serializeXml(settings, xmlFile, settings.getRom(), tableEntries, (s, name) -> getTableValue(s, name), add);
  }

  protected <T> void serializeXml(@NonNull T settings, @NonNull File xmlFile, String rom, List<String> tableEntries, SettingsGetter<T> getter, boolean add) throws VPinStudioException {
    try {
      ...
      if (add) {
        appendChild(settings, rom, tableEntries, getter, root, doc);
      }
      else {
  ...
  private <T> void removeChild(@NotNull T settings, String rom, List<String> tableEntries, SettingsGetter<T> getter, Node root, Document doc) {
    ...
    if (rom != null) {
      ...
      if (rootNodeByRom != null) {
        root.removeChild(rootNodeByRom);
      }
    }
  }
