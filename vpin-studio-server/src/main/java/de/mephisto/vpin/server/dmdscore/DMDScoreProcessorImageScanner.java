...
public class DMDScoreProcessorImageScanner implements DMDScoreProcessor {
    ...
    protected int[] blur(int[] inPixels, int width, int height, int radius) {
        ...
        src.getRaster().setDataElements(0, 0, width, height, inPixels);
        ...
        ConvertBufferedImage.convertTo(output, dst, true);

        int[] outPixels = new int[width * height];
        dst.getRaster().getDataElements(0, 0, width, height, outPixels);
        return outPixels;
    }
}
