...
public class IniArchiveAnalyzer {
  private final static Logger LOG = LoggerFactory.getLogger(IniArchiveAnalyzer.class);
  ...
  private static String analyzeRar(@NonNull File file) {
    ...
    try {
      ...
      for (ISimpleInArchiveItem item : inArchive.getSimpleInterface().getArchiveItems()) {
        ...
        if (item.getPath().toLowerCase().endsWith(".ini")) {
          fileFound = true;
          break;
        }
      }
    ...
    if (!fileFound) {
      return "The selected archive does not contain a \".ini\" file.";
    }
    return null;
  }
