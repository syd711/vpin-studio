...
public class VPSAssetsDialogController implements DialogController, AutoCompleteTextFieldChangeListener {
  ...
  private void refreshTableView(VpsTable vpsTable) {
    ...
    TablesSidebarVpsController.addSection(dataRoot, "PUP Pack", game, VpsDiffTypes.pupPack, vpsTable.getPupPackFiles());
    TablesSidebarVpsController.addSection(dataRoot, "Backglasses", game, VpsDiffTypes.b2s, vpsTable.getB2sFiles());
    TablesSidebarVpsController.addSection(dataRoot, "ALT Sound", game, VpsDiffTypes.altSound, vpsTable.getAltSoundFiles());
    TablesSidebarVpsController.addSection(dataRoot, "ALT Color", game, VpsDiffTypes.altColor, vpsTable.getAltColorFiles());
    TablesSidebarVpsController.addSection(dataRoot, "Sound", game, VpsDiffTypes.sound, vpsTable.getSoundFiles());
    TablesSidebarVpsController.addSection(dataRoot, "Topper", game, VpsDiffTypes.topper, vpsTable.getTopperFiles());
    TablesSidebarVpsController.addSection(dataRoot, "ROM", game, VpsDiffTypes.rom, vpsTable.getRomFiles());
    TablesSidebarVpsController.addSection(dataRoot, "Wheel Art", game, VpsDiffTypes.wheel, vpsTable.getWheelArtFiles());
    TablesSidebarVpsController.addSection(dataRoot, "POV", game, VpsDiffTypes.pov, vpsTable.getPovFiles());
