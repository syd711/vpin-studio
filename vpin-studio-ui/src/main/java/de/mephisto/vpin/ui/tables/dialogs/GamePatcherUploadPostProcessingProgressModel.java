...
public class GamePatcherUploadPostProcessingProgressModel extends ProgressModel<UploadDescriptor> {
  ...
  public GamePatcherUploadPostProcessingProgressModel(String title, UploadDescriptor uploadDescriptor) {
    super(title);
  ...
  public void processNext(ProgressResultModel progressResultModel, UploadDescriptor descriptor) {
    try {
      UploadDescriptor result = Studio.client.getPatcherService().proccessTablePatch(uploadDescriptor);
